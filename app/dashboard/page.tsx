...
function InterceptorInterface({ interventions, onAnalyze, onAccept, onIgnore }: any) {
    ...
    if (item?.viewMode === 'SUMMARY') return (
        <div className="relative w-full h-auto min-h-[200px] flex flex-col justify-end p-2 animate-in zoom-in-95 relative z-50">
            ...
            <div className="w-full h-auto min-h-fit bg-black/90 border-t-2 border-primary/50 backdrop-blur-2xl p-6 pb-8 flex flex-col gap-4 shadow-[0_-10px_30px_rgba(19,200,236,0.2)] rounded-xl">
                <div className="flex justify-between items-start">
                    <span className="text-[10px] uppercase tracking-[0.3em] text-primary/60 animate-pulse font-mono">
                        [ Signal Détecté ]
                    </span>
                    <Radar size={16} className="text-primary/40" />
                </div>

                {/* La zone du nom "User" : elle doit pouvoir passer sur plusieurs lignes */}
                <div className="text-xl font-mono text-primary leading-tight break-words font-bold">
                    {item.targetName || "IDENTIFICATION..."}
                </div>

                {/* Le bouton : on lui donne une taille fixe mais on le laisse respirer */}
                <button
                    onClick={() => onAnalyze(item.id)}
                    className="w-full py-5 bg-primary/10 border border-primary text-primary font-bold uppercase tracking-widest active:bg-primary/30 touch-manipulation transition-all rounded-lg text-xs"
                >
                    ANALYSER
                </button>
    ...
    if (item?.viewMode === 'DEEP_AUDIT') return (
        <div className="relative w-full h-auto min-h-[200px] flex flex-col justify-end p-2 animate-in slide-in-from-right relative z-50 transition-all">
            ...
            <div className="w-full h-auto min-h-fit bg-black/90 border-t-2 border-accent-amber/50 backdrop-blur-2xl p-6 pb-8 flex flex-col gap-4 shadow-[0_-10px_30px_rgba(255,176,32,0.15)] rounded-xl">
                <h2 className="text-accent-amber font-bold text-sm tracking-[0.2em] uppercase font-mono animate-pulse">
                    [ Audit_Terminé ]
                </h2>
                ...
                <div className="flex items-center gap-6">
                    <div className="relative">
                        <svg className="w-20 h-20 transform -rotate-90">
                            <circle cx="40" cy="40" r="35" stroke="currentColor" strokeWidth="3" fill="transparent" className="text-gray-800" />
                            <circle cx="40" cy="40" r="35" stroke="currentColor" strokeWidth="3" fill="transparent" strokeDasharray={219.8} strokeDashoffset={219.8 - (219.8 * item.matchScore) / 100} className="text-accent-amber" />
                        </svg>
                        <div className="absolute inset-0 flex items-center justify-center text-xl font-black text-white font-mono">{item.matchScore}%</div>
                    </div>

                    <div className="flex-1 text-left">
                        <p className="text-[10px] text-accent-amber/60 font-mono leading-tight">SYNERGIE_CALCULÉE</p>
                        <p className="text-xs text-slate-300 font-mono mt-1 italic">"{item.opportunities[0]}"</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3 w-full">
                    <button
                        ...
                        className="w-full py-5 bg-accent-amber/10 border border-accent-amber text-accent-amber font-bold active:bg-accent-amber/30 transition-all rounded-lg text-xs uppercase tracking-widest touch-manipulation shadow-[0_0_15px_rgba(255,176,32,0.2)]"
                    >
                        CONNECTER
                    </button>
